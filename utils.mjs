...
export function agregarSuperheroes(rutaOriginal, rutaNuevos) {
    const datosOriginales = fs.readFileSync(rutaOriginal, 'utf8'); // tomamos los datos del primer archivo
    const datosNuevos = fs.readFileSync(rutaNuevos, 'utf8'); // segundo archivo

    const superheroesOriginales = JSON.parse(datosOriginales);
    const nuevosSuperheroes = JSON.parse(datosNuevos);  

    //instanciamos a los Nuevos
